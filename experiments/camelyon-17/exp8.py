...
# root_dir = "../new-paradigm/new-10-client-1-server-10-ood-2-untrained-Adam/models"
# path = Path(root_dir)
# client_paths = [path/f"client-{i}" for i in range(8)]
# server_path = path/"server"
# distribution_clients = [0,1,2,3,4,5,6,7]
...
#         # print(cov.shape)
#         return torch.mm(x, y.T)/(torch.norm(x, dim = 1).reshape(-1, 1) * torch.norm(y, dim = 1).reshape(1, -1))
