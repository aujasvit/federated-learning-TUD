...
class InResize(INAugmentation):
    def __init__(self, resize_size: Tuple[int, int]) -> None:
        ...
        self.resize = Resize(self.resize_size, interpolation=InterpolationMode.BILINEAR)
        self.resize_label = Resize(
            self.resize_size, interpolation=InterpolationMode.NEAREST
        )
    ...
    def _augment(self, sample: Sample) -> Tuple[Tensor, Tensor]:
        ...
        for k, v in sample.label_dict.items():
            if v.shape == required_size_for_resizing:
                sample[k] = self.resize_label(v.unsqueeze(0).unsqueeze(0)).squeeze()
        return sample
