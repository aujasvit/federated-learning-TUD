...
class ImageFolderCustom(Dataset):
    ...
    def __init__(self, targ_dir: str, transform=None) -> None:
        ...
        df = pd.read_csv("/local/scratch/camelyon17/camelyon17_v1.0/metadata.csv")
        self.df = df
        self.train_client_indices = [0,1,3,4]
        ...
        random.seed(10)
        self.aug_types = [fr.DarkSpotsAugmentation(2), fr.FatAugmentation(2), fr.SquamousAugmentation(2), fr.ThreadAugmentation(2), fr.GaussianBlurAugmentation(sigma=5)]
        self.augs = []
        for i in range(len(self.center)):
            if self.center[i] in self.ood_client_indices:
                self.augs.append(random.choice(self.aug_types))
            else:
