...
class Server:
    def __init__(self,
        ...
        self.lr = lr
        self.device = device
        self.train_client_indices = [0,1,3,4,5,6,8,9]
        self.ood_client_indices = [3, 8]
        self.client_models = []
    ...
    def run(self,
        ...
        for epoch in tqdm(range(server_epochs)):
            ...
            for j in range(self.num_train_clients):
                ...
                writer.add_scalar(f"train_loss/client-{self.train_client_indices[j]}", results["train_loss"][-1], epoch)
                writer.add_scalar(f"train_acc/client-{self.train_client_indices[j]}", results["train_acc"][-1], epoch)
                writer.add_scalar(f"validate_loss/client-{self.train_client_indices[j]}", results["validate_loss"][-1], epoch)
                writer.add_scalar(f"validate_acc/client-{self.train_client_indices[j]}", results["validate_acc"][-1], epoch)

                attempts = 0
                while attempts < 30:
                    attempts += 1
                    try:
                        p = folder_path/f"models/client-{self.train_client_indices[j]}/"
